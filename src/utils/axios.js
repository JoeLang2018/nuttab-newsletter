...
_axios.interceptors.request.use(
  originConfig => {
    ...
    // step2: permission 处理
    // 如果是刷新令牌的请求，header需携带refresh_token
    ...
    } else { //如果是其他请求，则header需携带access_token
      // 有access_token
...
_axios.interceptors.response.use(
  async res => {
    // return res.data
    // console.log('res', res)
    let { code, message } = res.data.data // eslint-disable-line
    ...
    return new Promise(async (resolve, reject) => {
      ...
      // refreshToken相关，直接登出
      if (refreshTokenException(code)) { //10000, 10042, 10050, 10052
        setTimeout(() => {
